...
class ExperimentRecorder:
    ...
    def save_lr_hr_pred_comparison(
        ...
        def _ensure_batch_dim(tensor):
            if len(tensor.shape) == 3:  # [C, H, W] → [1, C, H, W]
                return tensor.unsqueeze(0)
            return tensor

        lr_tensors = _ensure_batch_dim(lr_tensors)
        hr_tensors = _ensure_batch_dim(hr_tensors)
        pred_tensors = _ensure_batch_dim(pred_tensors)

        num_samples = min(num_samples, len(lr_tensors), len(hr_tensors), len(pred_tensors))
        if num_samples == 0:
            raise ValueError("No samples to plot (num_samples=0)")

        tb_imgs = []
        for i in range(num_samples):
            lr_img = lr_tensors[i].cpu()
            hr_img = hr_tensors[i].cpu()
            pred_img = pred_tensors[i].cpu()

            if lr_img.dtype == torch.uint8:
                lr_img = lr_img.float() / 255.0
                hr_img = hr_img.float() / 255.0
                pred_img = pred_img.float() / 255.0

            combined_img = torch.cat([lr_img, pred_img, hr_img], dim=-1)
            tb_imgs.append(combined_img)

        if tb_imgs:
            tb_img = torch.cat(tb_imgs, dim=-2) 

            self.writer.add_image(
                tag=f"images/comparison_grid_epoch_{epoch}",
                img_tensor=tb_img,
                global_step=epoch,
                dataformats="CHW"
            )
