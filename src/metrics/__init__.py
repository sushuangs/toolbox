    ...
    def forward(self, sr, hr):
        ...
        for i, l in enumerate(self.loss):
            if l['function'] is not None:
                ...
                metric_list[l['type']] = effective_loss.item()
...
class MetricCalculator:
    ...
    def _validate_configs(self, configs):
        ...
        for cfg in configs:
            ...
            metric_type = cfg.get("type", "").strip()
            params = cfg.get("params", {}) or {}

            if not name:
                continue
            if metric_type not in SUPPORTED_METRICS:
                raise ValueError(f"{metric_type} not in {list(SUPPORTED_METRICS.keys())}")
            
            validated[name] = {
                "func": SUPPORTED_METRICS[metric_type],
